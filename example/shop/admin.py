...
class ProductAdmin(admin.ModelAdmin):
    ...
    list_display = ["name", "price_display", "stock", "is_active", "created_at"]
    ...
    fieldsets = (
        ("Product Information", {"fields": ("name", "description", "image")}),
        ("Pricing & Stock", {"fields": ("price", "currency", "stock", "is_active")}),
...
class OrderAdmin(IyzicoPaymentAdminMixin, admin.ModelAdmin):
    ...
    fieldsets = (
        ("Order Information", {"fields": ("order_number", "user", "order_status", "notes")}),
        ("Shipping Details", {"fields": ("shipping_address", "shipping_city", "shipping_country")}),
        (
            "Payment Information",
            {
                "fields": (
        ...
        (
            "Raw API Response",
            {"fields": ("get_raw_response_display_admin",), "classes": ("collapse",)},
        ),
        ("Timestamps", {"fields": ("created_at", "updated_at"), "classes": ("collapse",)}),
    )
    ...
    def installment_display(self, obj):
        ...
        if obj.has_installment():
            if obj.installment_rate == 0:
                return format_html(
                    '<span style="color: green;">{}x (0% Interest)</span>', obj.installment_count
                )
            return format_html("{}x ({}% Interest)", obj.installment_count, obj.installment_rate)
        return "-"
...
class OrderItemAdmin(admin.ModelAdmin):
    ...
    list_display = ["order", "product", "quantity", "price", "currency", "total_display"]
